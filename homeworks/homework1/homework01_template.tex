...
\begin{enumerate}
  ...
  \item For convex loss functions (i.e. with a bowl
    shape\footnote{More formally $f$ is convex on $[a,b]$ if $\forall
      ...
      (1-\lambda)\vec{x}_2) \le \lambdaf(\vec{x}_1) + (1-\lambda) f(\vec{x}_2)$}), batch gradient descent is guaranteed to eventually converge to the global optimum while stochastic gradient descent is not.
