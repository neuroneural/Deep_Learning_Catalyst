...
\problem{5} Compare the following metrics and explain which one is better.
...
Learn a simple and efficient procedure for obtaining analytic
derivatives of complex matrix/vector objectives. Based on the
exposition in Appendix III of
\href{https://arxiv.org/abs/1911.04048}{https://arxiv.org/abs/1911.04048}
manually differentiate the following objective function:
$$
\displaystyle \frac{1}{4n(n-1)} \vec{1}^T \log \left( (\vec{x}\odot \vec{x}) \vec{1}^T + \vec{1}(\vec{x}\odot \vec{x})^T - 2\vec{x}\vec{x}^T + \mathbf{I}\right) \vec{1},
$$
where $\mathbf{I}$ is the identity matrix, $\vec{1}$ is appropriately
sized vector of ones, and $\odot$ denotes Hadamard element-wise
product.
List each and every one of your step. You may type them down by hand on
paper very clearly and submit a  photo. {\bf The answer expression for
  $\dot{\vec{x}}$    must    be   typeset    electronically    though}
\end{document}
