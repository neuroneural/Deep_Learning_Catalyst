...
\problem{20} Technically we need to compute the gradient with respect to $\mathbf{W}_i$, the linear transform (or parameter) matrix (tensor) for layer $i$. Yet, we are computing $\frac{\partial\ell}{\partial\vec{x}}$, gradient of the loss $\ell$ with respect to the input $\vec{x}$. How come?
